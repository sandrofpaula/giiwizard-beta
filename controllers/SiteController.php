...
use app\models\JsonForm; // Adicione o novo modelo
...
class SiteController extends Controller
{
    ...
    private function extractErrorCode($exception)
    {
        if ($exception instanceof \PDOException) {
            $errorInfo = $exception->errorInfo;
            if (isset($errorInfo[2]) && strpos($errorInfo[2], 'ORA-') !== false) {
                preg_match('/ORA-\d+/', $errorInfo[2], $matches);
                if (!empty($matches)) {
                    return $matches[0];
                }
            } elseif (isset($errorInfo[1])) {
                return $errorInfo[1];
            }
        } elseif ($exception instanceof Exception) {
            if (isset($exception->errorInfo[2]) && strpos($exception->errorInfo[2], 'ORA-') !== false) {
                preg_match('/ORA-\d+/', $exception->errorInfo[2], $matches);
                if (!empty($matches)) {
                    return $matches[0];
                }
            } elseif (isset($exception->errorInfo[1])) {
    ...
    private function getDbErrorMessage($errorCode)
    {
        $configFile = Yii::getAlias('@app/web/data/db-error-codes.json');
        if (!file_exists($configFile)) {
            return null;
        }
        $errorMessages = json_decode(file_get_contents($configFile), true);
    ...
    {
        try {
            // Tente conectar ao banco de dados ou executar uma consulta
            $db = Yii::$app->db;
            $db->open(); // Tenta abrir a conexão com o banco de dados
            
            // Código adicional aqui, se necessário
            
        }  catch (\Exception $e) {
            $errorCode = $this->extractErrorCode($e);
            $errorDetails = $this->getDbErrorMessage($errorCode);

            if ($errorDetails) {
                $message = "{$errorDetails['message']} (Código: {$errorDetails['code']}, Categoria: {$errorDetails['categoria']}, Banco de Dados: {$errorDetails['bancoDeDados']})";
                return $this->render('error', [
                    'name' => $errorDetails['name'],
                    'message' => $message,
                ]);
            } else {
                throw $e; // Re-lança a exceção se não for um dos erros tratados
            }
        }
    ...
    {
        return $this->render('/icones/icones-bootstrap');
    }
    public function actionIconesFontAwesome()
    {
        return $this->render('/icones/icones-font-awesome');
    }
    public function actionJsonFormatter()
    {
        $model = new JsonForm();
        $formattedJson = null;
        ...
        return $this->render('/extra/json-formatter', [
            'model' => $model,
    ...
    protected function formatJson($json)
    {
        ...
        if (json_last_error() == JSON_ERROR_NONE) {
            // Re-encode o JSON com formatação
            return json_encode($decoded, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
        } else {
            // Retorna uma mensagem de erro se o JSON for inválido
